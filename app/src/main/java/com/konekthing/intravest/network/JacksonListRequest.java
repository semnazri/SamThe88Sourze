...
public class JacksonListRequest<T> extends Request<List<T>> {
    ...
    protected final Listener<List<T>> mListener;
    protected final Class<T> mClazz;
    private final Object mRequestBody;

    JacksonListRequest(Builder<T> builder) {
    ...
    public interface _Listener<T> {
        _ErrorListener<T> listener(Listener<List<T>> listener);
    }
    ...
    public static class Builder<T> implements _Builder<T>, _Class<T>, _ErrorListener<T>,
        ...
        @Override
        public _ErrorListener<T> listener(Listener<List<T>> listener) {
        ...
        @Override
        public JacksonListRequest<T> build() {
            return new JacksonListRequest<T>(this);
        }
    ...
    protected Response<List<T>> parseNetworkResponse(NetworkResponse response) {
        try {
            List<T> jsonList = JSON.std.listOfFrom(mClazz, response.data);
            return Response.success(jsonList, HttpHeaderParser.parseCacheHeaders(response));
